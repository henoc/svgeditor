...
document.onmousedown = function (ev) {
    ev.stopPropagation();
    var x = ev.clientX - common_1.svgroot.clientLeft;
    var y = ev.clientY - common_1.svgroot.clientTop;
    rectangle = {
        id: utils_1.uuid(),
        start: utils_1.Point.of(x, y),
        end: utils_1.Point.of(x, y)
    };
    var rectSvgText = "<rect x=\"" + x + "\" y=\"" + y + "\" width=\"0\" height=\"0\" id=\"" + rectangle.id + "\"/>\n";
    common_1.svgroot.insertAdjacentHTML("beforeend", rectSvgText);
};
document.onmousemove = function (ev) {
    ev.stopPropagation();
    if (rectangle) {
        rectangle.end = utils_1.Point.of(ev.clientX - common_1.svgroot.clientLeft, ev.clientY - common_1.svgroot.clientTop);
        var rectElem = document.getElementById(rectangle.id);
        ...
        svgutils_1.deform(rectElem).seta("x", String(leftUp.x));
        svgutils_1.deform(rectElem).seta("y", String(leftUp.y));
        svgutils_1.deform(rectElem).seta("width", String(rightDown.x - leftUp.x));
        svgutils_1.deform(rectElem).seta("height", String(rightDown.y - leftUp.y));
    }
};
