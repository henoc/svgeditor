...
scale ratio elems = 
  let
    center = getCenter elems
    scaled = List.map (Shape.scale ratio) elems
  in
...
scale2 offset ratio elems =
  let
    fixedPoint = getOffsettedCenter offset elems
    centerScaled = List.map (Shape.scale ratio) elems
  in
